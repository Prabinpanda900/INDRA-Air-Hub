...
# 🗺️ EXPANDED AUTHENTIC CPCB REGISTRY
MASTER_CITY_COORDINATES = {
    ...
    "ghaziabad": (28.6692, 77.4538), "noida": (28.5355, 77.3910), "gurugram": (28.4595, 77.0266),
    ...
    "siliguri": (26.7271, 88.3953), "howrah": (22.5731, 88.2636), "shillong": (25.5788, 91.8833), 
    "itanagar": (27.0844, 93.6053), "dimapur": (25.9094, 93.7266), "imphal": (24.8170, 93.9368), 
    "aizawl": (23.7271, 92.7176), "gangtok": (27.3314, 88.6138)
}
...
def inject_supplementary_sensor_grid(df_live, geo_india):
    # Core operational nodes ensuring fallback coverage
    supplementary_nodes = [
    ...
    # STRICTLY NO RANDOM GRID GENERATION - Returning ONLY true coordinates & fallbacks
...
def get_gee_satellite_matrix(pollutant_theme, geo_india):
    # Simulated Satellite sweeps are isolated to this layer
    simulated_points = []
...
with layout_panel_left:
    ...
    # --- GPS TRACKING AND GEODESIC NODE RESOLUTION ---
    col_search, col_track = st.columns([3.5, 1])
    ...
    if geo_data and geo_data.get('latitude') and geo_data.get('longitude'):
        ...
        if "last_gps" not in st.session_state or st.session_state.last_gps != current_gps:
            ...
            min_dist = float('inf')
            nearest_node = search_pool[0]
            
            for city_name in search_pool:
                c_key = city_name.lower().strip()
                if c_key in MASTER_CITY_COORDINATES:
                    c_lat, c_lon = MASTER_CITY_COORDINATES[c_key]
                    
                    # High-precision Haversine computational loop
                    R = 6371.0
                    ...
                    if km_distance < min_dist:
                        min_dist = km_distance
    ...
    with col_search:
        if "city_selector" not in st.session_state or st.session_state.city_selector not in search_pool:
            ...
            st.session_state.city_selector = search_pool[default_index]
